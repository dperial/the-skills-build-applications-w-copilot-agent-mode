  ...
  return (
    <div className="container mt-4">
      <div className="d-flex align-items-center justify-content-between mb-3">
        <h2 style={{ fontSize: "1.5rem", wordBreak: "break-word" }}>Équipes</h2>
      ...
      {showForm && (
        <form className="mb-3" onSubmit={handleAddTeam}>
      ...
      <div className="table-responsive" style={{ overflowX: "auto" }}>
        <table className="table table-striped" style={{ minWidth: 260 }}>
          ...
          <tbody>
            {teams.map(team => (
              <tr key={team.id} style={{ fontSize: "0.95rem" }}>
                <td style={{ wordBreak: "break-word" }}>{editId === team.id ? (
                  <form onSubmit={e => {
                    e.preventDefault();
                    fetch(endpoint + team.id + '/', {
                      method: 'PUT',
                ...
                <td>
                  {editId === team.id ? null : (
                    <>
                      <button className="btn btn-outline-secondary btn-sm rounded-circle me-2" style={{ minWidth: 36, minHeight: 36, padding: 6 }} onClick={() => handleEdit(team)}>
                        <FaEdit />
                      </button>
                      <button className="btn btn-outline-danger btn-sm rounded-circle" style={{ minWidth: 36, minHeight: 36, padding: 6 }} onClick={() => handleShowDeleteModal(team.id)}>
                        <FaTrash />
                      </button>
                    </>
                  )}
